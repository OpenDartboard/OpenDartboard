...
{
    cout << "Calibrating camera " << cameraIdx + 1 << "..." << endl;
    ...
    // Debug logging
    cout << "DEBUG-CALIB: Processing camera " << cameraIdx + 1 << endl;
    ...
    if (found && radius > 25)
    {
        ...
        *this = createStandardCalibration(center, radius, orientation, cameraIdx);
        ...
        // Log calibration
        cout << "Camera " << cameraIdx + 1 << " calibrated: center=("
    ...
    {
        cout << "Failed to calibrate camera " << cameraIdx + 1 << endl;
        return false;
    }
}

// Moved from GeometryDetector - Simplified Color-Based Dartboard Detection
bool DartboardCalibration::findDartboardCircle(const cv::Mat &frame, cv::Point &center, double &radius, double &orientation, int camera_idx, bool debug_mode)
{
    ...
    int initialRoiRadius = min(frame.cols, frame.rows) / 2.0;
    ...
    cout << "Camera " << camera_idx + 1 << ": Center=(" << center.x << "," << center.y
         << "), radius=" << radius << ", confidence=" << bestScore << endl;

    return true;
}

// Moved from GeometryDetector - Create a standard calibration
